<template>
    <div>
        ...
        <el-select style="margin-left:15px;" v-model="selectedAlgValue" placeholder="请选择算法" @change="changeAVal">
            ...
            <el-option value="5" label="明火检测"></el-option>
        </el-select>
        <el-button style="margin-left:10px;" @click="changeAlgorithm" type="primary" plain>切换</el-button>
        <div class="line"></div>
        <div class="vid" >
            <!-- <img :src="imageSrc" ref="image" @error="onImageError"/> -->
...
<script>
    ...
    methods: {
        ...
        changeAlgorithm(){
            ...
            if(algorithm==="1"){
                this.gotoAlgorithmOne()
            }else if(algorithm==="2"){
                this.gotoAlgorithmTwo()
            }else if(algorithm==="3"){
                this.gotoAlgorithmThree()
            }else if(algorithm==="4"){
                this.gotoAlgorithmFour()
            }else if(algorithm==="5"){
                this.gotoAlgorithmFive()
            }else if(algorithm==="6"){
                this.gotoAlgorithmSix()
            }
        },
        onImageError() {
         // 图片404时展示这张图
        ...
        async gotoCameraOne(){
            ...
            try {
                await this.loadImage(image1, url1, 60000); // 设置加载超时时间为 6000 毫秒（60秒）
            } catch (error) {
                console.error(error);
            // 视频流加载失败
            // 展示超时对应的图片
        ...
        async gotoCameraTwo() {
            ...
            try {
            await this.loadImage(image2, url2, 60000); // 设置加载超时时间为 6000 毫秒（60秒）
            } catch (error) {
                console.error(error);
            // 视频流加载失败
            // 展示超时对应的图片
        ...
        async gotoCameraThree() {
            ...
            try {
                await this.loadImage(image3, url3, 60000); // 设置加载超时时间为 6000 毫秒（60秒）
            } catch (error) {
                console.error(error);
                // 视频流加载失败
                // 展示超时对应的图片
