<template>
  <div>
      ...
      <el-select style="margin-left:15px;" v-model="selectedAlgValue" placeholder="请选择算法" @change="changeAVal">
          ...
          <el-option value="3" label="情绪识别"></el-option>
          <el-option value="4" label="交互识别"></el-option>
          <el-option value="5" label="跌倒检测"></el-option>
          <el-option value="6" label="明火烟雾检测"></el-option>
          <el-option value="7" label="积水检测"></el-option>
          <el-option value="8" label="房屋渗水检测"></el-option>
      </el-select>
  ...
  methods: {
    ...
    async changeCA(){
      ...
      if(algorithm==="1"){
        ...
        const url = `${server_url}?video_url=${video_url}&px1=${this.px1}&px2=${this.px2}&px3=${this.px3}&px4=${this.px4}&py1=${this.py1}&py2=${this.py2}&py3=${this.py3}&py4=${this.py4}`;
        try {
      ...
      }else if(algorithm==="2"||algorithm==="3"||algorithm==="4"){ // 人脸、情绪、交互
      ...
      }else if(algorithm==="6"){ // 明火烟雾
        const server_url = 'http://43.143.247.127:8090/api/v1/cv/fire';
        const image = this.$refs.image;
        const video_url = camera;
        const url = `${server_url}?video_url=${video_url}`;
        try {
      ...
      }else if(algorithm==="7"){ // 积水
        const server_url = 'http://43.143.247.127:8090/api/v1/cv/water';
        const image = this.$refs.image;
        const video_url = camera;
        const url = `${server_url}?video_url=${video_url}`;
        try {
      ...
      }else if(algorithm==="8"){ // 渗水
        const server_url = 'http://43.143.247.127:8090/api/v1/cv/seepage';
        const image = this.$refs.image;
        const video_url = camera;
        const url = `${server_url}?video_url=${video_url}`;
        try {
    ...
    open() {
      ...
      this.px1 = this.positions[0].left / parseFloat(width)
      this.py1 = this.positions[0].top / parseFloat(height)
      this.px2 = this.positions[1].left / parseFloat(width)
      this.py2 = this.positions[1].top / parseFloat(height)
      this.px3 = this.positions[2].left / parseFloat(width)
      this.py3 = this.positions[2].top / parseFloat(height)
      this.px4 = this.positions[3].left / parseFloat(width)
      this.py4 = this.positions[3].top / parseFloat(height)
